...
import property_predictor_test
...
data_MP = './MP_clean_canonize_cut.csv'


# save_uri='./SMILES(RNN).ckpt'
...
for random_seed in range(test_times):
    ...
    smiles_MP = data.values[:,0] #0: SMILES
    ...
    Y = np.asarray(data.values[:,1:], dtype=np.float32)  # 1.IE   2.EA 
    ...
    with model.session:
        model.train(trnX_L=trnX_L, trnXs_L=trnXs_L, trnY_L=trnY_L,
                    valX_L=valX_L, valXs_L=valXs_L, valY_L=valY_L)
