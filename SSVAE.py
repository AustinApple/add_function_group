...
class Model(object):
    ...
    def __init__(self, seqlen_x, dim_x, dim_y, dim_z=100, dim_h=250, n_hidden=3, batch_size=200, beta=10000., char_set=[' ']):
        ...
        with tf.name_scope('label') as scope:
            ...
            self.xs_L = tf.placeholder(tf.float32, [None, self.seqlen_x, self.dim_x], name="xs_L")
    ...
    def train(self, trnX_L, trnXs_L, trnY_L, trnX_U, trnXs_U, valX_L, valXs_L, valY_L, valX_U, valXs_U):
        ...
        print(n_batch)
        print(" # of training label data :",n_batch*batch_size_L)
        print("# of training unlabel data :",n_batch*batch_size_U)
        ...
        summary_objL = tf.summary.scalar('objL',objL)
        summary_objU = tf.summary.scalar('objU',objU)
        summary_objYpred_MSE = tf.summary.scalar('objYpred_MSE',objYpred_MSE)
        ...
        writer = tf.summary.FileWriter("Tensorboard_zinc_MP_30W/random_0", graph=self.session.graph)
        ...
        for epoch in range(300):
            ...
            for i in range(n_batch):
                ...
                trn_res = self.session.run([train_op, cost, objL, objU, objYpred_MSE,summary_merge_trn],
            ...
            writer.add_summary(trn_res[5], epoch)
            val_res = []
            for i in range(10):
                print(i)
                ...
                val_res.append(self.session.run([cost_val, objL_val, objU_val],
                ...
                val_tensorboard = self.session.run(summary_merge_val, feed_dict = {self.x_L: valX_L[start_L:end_L], self.xs_L: valXs_L[start_L:end_L], self.y_L: valY_L[start_L:end_L],
                                self.x_U: valX_U[start_U:end_U], self.xs_U: valXs_U[start_U:end_U]})
                writer.add_summary(val_tensorboard, epoch * int(10)+ i) 
            ...
            self.saver.save(self.session,'./model_SSVAE_random_0/'+str(epoch))
            if epoch > 20 and np.min(val_log[0:epoch-10]) * 0.99 < np.min(val_log[epoch-10:epoch+1]):
                print('---termination condition is met')
                break
        
    #def reload(self,trnX_L, trnXs_L, trnY_L, trnX_U, trnXs_U, valX_L, valXs_L, valY_L, valX_U, valXs_U, model_name):
    def reload(self, model_name,Y):    
